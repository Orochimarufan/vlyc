"""
/*****************************************************************************
 * vlyc :: input_slider.py : VolumeSlider and SeekSlider
 ****************************************************************************
...
#private:
    ...
    def __init__(self, parent, i_step, b_softamp, psz_colors):
        ...
        while ("" in colorList):
            colorList.remove("")
        if (len(colorList) < 12):
            for _ in range(12 - len(colorList)):
        ...
        def add_color(rang, c1, c2, c3):
            return gradient.setColorAt(rang, QtGui.QColor(c(c1), c(c2), c(c3)))
        
        def desaturate(c):
            c.setHsvF(c.hueF(), 0.2, 0.5, 1.0)
            return c
        
        def add_desaturated_color(rang, c1, c2, c3):
            return gradient2.setColorAt(rang, desaturate(QtGui.QColor(c(c1), c(c2), c(c3))))
        
        def add_colors(rang, c1, c2, c3):
            add_color(rang, c1, c2, c3)
            add_desaturated_color(rang, c1, c2, c3)
