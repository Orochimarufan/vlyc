"""
/*****************************************************************************
 * vlyc :: widgets.py : Custom widgets for the main interface
 ****************************************************************************
...
class TimeLabel(ClickableQLabel):
    ...
    def __init__(self, displayType):
        ...
        if (displayType == self.Display.Elapsed):
            ...
            self.setText(" --:-- ")
            self.setToolTip(QtCore.QCoreApplication.translate("TimeLabel",
    ...
    def setDisplayPosition(self, f_pos, i_t=None, i_length=None):
        ...
        if (i_t is None and i_length is None):
            ...
            psz_time = secstotimestr(self.cachedLength - time
                    if (self.b_remainingTime and self.cachedLength) else time)
            timestr = "{0}{1}/{2}".format("-" if (self.b_remainingTime and i_length) else "",
                                          psz_time,
